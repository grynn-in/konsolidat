-- assert_scenario_rows_declared: must FAIL with exactly 1 result (PR #211 review 2, konsolidat#206).
-- Entity ZZE1 uploads a FY2026 P1 trial balance, so gold_scenario_trial_balance carries GL rows
-- (data_source 'gl'). The site declares TWO active actual-type scenarios, ACTUAL and ZZ_ACT, so the model
-- cannot pick one and keeps the fallback 'ACTUAL' on the GL rows. ACTUAL is declared, active and of type
-- actual, so a per-row check alone passes; the test must still name the ambiguity:
-- ('ACTUAL,ZZ_ACT', '', 'several_actual').
-- No budget lines, so no budget branch rows.
--
...
-- epm_gold.budget_annual_input, epm_gold.budget_monthly_input, epm_gold.entity_fiscal_calendars,
-- epm_gold.spread_profiles, epm_staging.dimension_mappings.
--
