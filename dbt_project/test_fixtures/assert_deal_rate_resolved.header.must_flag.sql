-- Must-flag fixture for assert_deal_rate_resolved (konsolidat#198 row J13; PR #203 second review 1): a submitted
-- deal with NO consideration lines whose header total_consideration is stated in a currency the group has no
-- governed Closing rate for, while the document does have one acquisition-costs line (in the group currency).
-- Gate: `+gold_business_combination_journal assert_deal_rate_resolved` must show `FAIL 1 assert_deal_rate_resolved`
-- / `Got 1 result` (GBP -> USD, FY2026 P3, kind business_combination), and gold_business_combination_journal must
-- hold NO row for the deal: with no consideration lines the journal falls back to the header figure and INNER JOINs
-- its rate (header_consideration_rated), so the unrated header drops the deal; assert_goodwill_calculated FAIL 1
-- is the predicted side effect (a deal with consideration and no goodwill or bargain-gain line).
-- Before row J13 the guard counted consideration AND cost rows in one line_counts, so this document (one cost
-- line, no consideration line) was treated as "has lines" and its header currency was never checked: the guard
-- PASSed while the journal posted nothing — a silent drop. The guard now gates the header check on consideration
-- rows only for a business_combination (proceeds rows for a business_disposal), mirroring the journals.
--
-- business_combination_100pct.sql verbatim except: the header's consideration_currency is 'GBP' (8,300 stays
-- konsol's Result figure), the consideration table holds no line for the deal, and the costs table holds one
-- line: Legal 120 USD (the group currency translates at 1 and needs no rate). Only EUR -> USD Closing/Average
-- rates exist for FY2026 P3.
--
-- The six deal tables do not exist live yet: the fixture creates the six the three journals read with konsol's
...
VALUES
  ('BC-ZZG-ZZS-2026-03-15', 'ZZG', 'ZZS', '2026-03-15', 100, 'GBP', 8300, 650, 930, 6720, 0, 0, 'ZZ-OP-ZZS-1', 'partial');
INSERT INTO epm_staging.business_combination_costs
  (parent, idx, kind, amount, currency, description)
VALUES
  ('BC-ZZG-ZZS-2026-03-15', 1, 'Legal', 120, 'USD', 'Legal fees in the group currency: rated at 1, the only line the document has');
