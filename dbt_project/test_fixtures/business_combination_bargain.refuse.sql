-- Acquisition journal, a BARGAIN PURCHASE under `bargain_purchase = 'Refuse'` (konsolidat#198 row J4, design §1a/§4 line 3b):
-- `+gold_business_combination_journal assert_bargain_purchase_refused` must BUILD and the test must FAIL with
-- `Got 1 result`: the journal posts NOTHING for the deal and the test names it (deal, group, bargain amount).
--
-- The same deal as business_combination_bargain.sql: group ZZG (USD) acquires 100% of ZZS (EUR) on 2026-03-15
-- (FY2026 P3) for 1,200 USD cash against fair-value net assets of 1,580 (book 650 + FVA 930), a bargain of 380.
-- The only difference is the root row's policy, `bargain_purchase = 'Refuse'`: the group does not accept a
-- gain on acquisition (IFRS 3.36 asks for a reassessment first), so the warehouse refuses to post the deal and
-- the build stops on assert_bargain_purchase_refused until konsol's Business Combination is corrected.
...
VALUES
  ('ZZG', '', 'ZZ Group', 'USD',
   'partial', 'IFRS', 'Impairment only', 'Expense', 'Off', 'Refuse',
...
VALUES
  ('BC-ZZG-ZZS-2026-03-15', 'ZZG', 'ZZS', '2026-03-15', 100, 'USD', 1200, 650, 930, 0, 380, 0, 'ZZ-OP-ZZS-1', 'partial');
...
VALUES
  ('BC-ZZG-ZZS-2026-03-15', 1, 'Cash', 1200, 'USD', '2026-03-15', 'Cash paid at completion');
