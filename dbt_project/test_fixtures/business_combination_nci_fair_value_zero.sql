...
-- M4 a partial-share 'full' deal with nci_fair_value <= 0 and a RATED currency found its row there (0 x rate = 0),
-- passed figures_raw's rate check and posted Cr NCI 0 (dropped as a zero line) with goodwill 8,300 - 1,580 = 6,720,
-- understated by the minority's missing fair value. nci_fair_value_rated now also requires nci_fair_value > 0, so
-- such a deal has no row there and figures_raw drops it; the J7 guard names the missing nci_fair_value.
--
-- assert_deal_rate_resolved.nci_fair_value_zero.must_flag.sql's deal, verbatim except the group has a governed
-- ZZX -> USD Closing (and Average) rate 1.25 for FY2026 P3, so the only reason the deal posts nothing is its
-- undeclared nci_fair_value.
--
-- The deal tables do not exist live yet: the fixture creates the six the journals and the guards read with
...
VALUES
  ...
  ('USD', 'ZZX', 2026, 3, 'Closing', 1.25, 'ZZ-GER-2026-03'),
  ('USD', 'ZZX', 2026, 3, 'Average', 1.25, 'ZZ-GER-2026-03');
