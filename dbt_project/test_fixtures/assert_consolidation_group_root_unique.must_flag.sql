-- Must-flag fixture for assert_consolidation_group_root_unique (konsolidat#198 row J10, design §1; PR #203 review 6):
...
-- `Got 1 result` on these rows.
--
-- business_combination_100pct.sql verbatim, except that the root row of group ZZG (data_area_id = '', where konsol
-- syncs the policy and the accounts) is inserted TWICE, identical. Before row J10 the three journals joined the
-- root row as it stood, so a duplicate doubled every line of every journal (10 acquisition lines instead of 5,
-- still summing to 0, so no balance test caught it). Since row J10 group_policy aggregates the root row with any()
-- per column, and this test names the duplicate: it reads only consolidation_groups, so dbt build runs it FIRST
-- and its FAIL SKIPs the three journals and every test on them; the build stops at the declaration.
--
