-- Must-flag fixture for assert_disposal_accounts_declared (konsolidat#198 row J10, design §1/§5; PR #203 review 2):
-- `+gold_business_disposal_journal assert_disposal_accounts_declared assert_acquired_balances_complete
--  assert_disposal_has_balances assert_consolidation_group_root_unique` must BUILD and the test must FAIL with
-- `Got 2 results` on these rows.
--
-- business_disposal.sql verbatim, except the root row of group ZZG, which now lacks or misdeclares two accounts the
-- disposal journal posts to (one flagged row per disposal and field):
--   disposal_gain_loss_account  ''        -> missing (the balancing line of every disposal journal)
--   disposal_proceeds_account   'ZZ1099'  -> not a posting account of the chart (ZZCOA has no ZZ1099)
-- goodwill_account and fair_value_adjustment_account stay declared; nci_account is not required here (100% sold,
-- nothing retained). Left alone, the journal would post the gain to main_account '' and the proceeds to ZZ1099:
-- the silent posting this guard stops. The test reads only the sources and the chart, so dbt build runs it BEFORE
-- gold_business_disposal_journal and its FAIL SKIPs the disposal journal and every test on it; the acquisition
-- journal (which does not read business_disposals) still builds and its 5 lines are unchanged.
--
-- The J2 deal (ZZG acquires 100% of ZZS on 2026-03-15 for 8,300 USD; goodwill 6,720, FVA 930) is sold on
-- 2027-03-15 for 9,000 USD, exactly as business_disposal.sql describes.
--
