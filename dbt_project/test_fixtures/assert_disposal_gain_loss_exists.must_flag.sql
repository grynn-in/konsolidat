...
-- retained_interest_pct 20). gold_business_disposal_journal posts only full disposals (retained_interest_pct = 0):
-- the remeasurement of a retained interest is out of scope (design §5, §7), so the model posts NOTHING for this
-- deal, and the submitted disposal has no `gain_loss` line. assert_disposal_gain_loss_exists must name it: a
-- submitted disposal the warehouse did not account for is an error the accountant has to see, not silence.
--
-- Everything else is business_disposal.sql verbatim: group ZZG (USD; accounts ZZ1800 goodwill | ZZ1900 FVA |
-- ZZ3500 investment | ZZ3400 NCI | ZZ4900 bargain gain | ZZ4950 disposal gain/loss | ZZ1000 proceeds |
-- ZZ6900 amortisation expense | ZZ6950 acquisition costs), entity ZZS (EUR) acquired 2026-03-15 for 8,300 USD
-- (goodwill 6,720, FVA 930), one period-end-balance TB at FY2027 P3, EUR->USD 1.0 at both P3s, calendar
-- FY2026-FY2027, the six deal tables CREATEd with konsol's exact DDL (clickhouse/init-db.sql, pinned by
-- tests/test_deal_tables_ddl.py), the policy/flag/basis columns ALTERed in first, and the empty TB-lineage tables
-- named to exist: epm_raw.general_journal_account_entry_bi_entities, epm_raw.general_journal_entry_bi_entities,
-- epm_raw.ledgers, epm_raw.legal_entities, epm_raw.fiscal_calendar_years, epm_gold.entity_fiscal_calendars,
...
VALUES
  ('BD-ZZG-ZZS-2027-03-15', 'ZZG', 'ZZS', '2027-03-15', 80, 20, 'USD', 9000, 'ZZ-OP-ZZS-1');
