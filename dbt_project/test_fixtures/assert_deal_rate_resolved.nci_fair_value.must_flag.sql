-- Must-flag fixture for assert_deal_rate_resolved (konsol#204, row M2b): a submitted 80% deal that measures NCI
-- at 'full' declares its nci_fair_value in the header's consideration_currency, which the group has no governed
-- Closing rate for, while every consideration line is in the group currency.
-- Gate: `+gold_business_combination_journal assert_deal_rate_resolved` must show `FAIL 1 assert_deal_rate_resolved`
-- (ZZX -> USD, FY2026 P3, kind business_combination). Predicted side effects: `FAIL 1
-- assert_acquisition_accounts_declared` / `Got 1 result` (the J12 share refusal of every deal below 100%, which
-- stays: konsolidat#204) and, because that guard reads only the sources, the journal SKIPs in this gate.
-- Why the guard must name the pair: since row M2 the journal translates the declared NCI fair value at the deal's
-- consideration_currency Closing rate (nci_fair_value_rated, an INNER JOIN on deal_rates), and figures_raw drops a
-- partial-share 'full' deal whose NCI fair value found no rate (a missing rate stops the deal, it never zeroes the
-- NCI). Before row M2b the guard asked for the header currency only when the deal fell back to the header (no
-- consideration lines, header amount not zero), so this deal (one USD consideration line) was never checked for
-- ZZX: the guard PASSed while the journal would post nothing — a silent drop. The guard now also needs
-- (consideration_currency -> group currency) when nci_measurement = 'full', share_acquired_pct < 100 and
-- nci_fair_value > 0.
--
-- business_combination_80pct.sql's ZZH full deal alone, verbatim except: the header's consideration_currency is
-- 'ZZX' (8,300 stays konsol's Result figure). The consideration line stays 8,300 USD (the group currency
-- translates at 1 and needs no rate); nci_fair_value 1,900 is stated in ZZX. Only EUR -> USD Closing/Average
-- rates exist for FY2026 P3.
--
-- The six deal tables do not exist live yet: the fixture creates the six the journals and the guard read with
-- konsol's exact DDL (clickhouse/init-db.sql, pinned by tests/test_deal_tables_ddl.py), then adds the two
-- konsol#204/#205 columns the live business_combinations may not have. The live consolidation_groups,
-- main_accounts and submission control may predate their policy/flag/basis columns: add them first. The
-- journal reads gold_trial_balance, whose lineage reads these tables, empty on a trial-balance-only site with
-- no upload, so they are named here to exist (empty): epm_raw.trial_balance_submissions,
...
VALUES
  ('BC-ZZH-ZZT-2026-03-15', 'ZZH', 'ZZT', '2026-03-15', 80, 'ZZX', 8300, 650, 930, 8620, 0, 1900, 'ZZ-OP-ZZT-1', 'full', 1900);
...
VALUES
  ('BC-ZZH-ZZT-2026-03-15', 1, 'Cash', 8300, 'USD', '2026-03-15', 'Cash paid at completion, in the group currency: rated at 1');
