-- Must-flag fixture for assert_acquired_balances_complete (konsolidat#198 row J10, design §2/§3; PR #203 review 5):
-- `+gold_business_disposal_journal assert_disposal_accounts_declared assert_acquired_balances_complete
--  assert_disposal_has_balances assert_consolidation_group_root_unique` must BUILD and the test must WARN with
-- `Got 2 results` on these rows.
--
-- business_combination_100pct.sql verbatim, except konsol's acquired balance sheet of the J2 deal, which is now
-- incomplete in two ways (one row per deal and problem):
--   idx 1  main_account 'ZZ1199' (was ZZ1100)  -> not a posting account of the chart (ZZCOA has no ZZ1199)
--   idx 5  ZZ3100 book_amount -500 (was -550)  -> the sheet sums to +50, not 0 (200 + 600 - 150 - 100 - 500)
-- Both are silent in gold_business_combination_journal: it eliminates the equity rows the chart flags is_equity
-- (an unknown account is not equity, so its row is ignored) and takes net assets as -(the equity eliminated), so
-- here it books equity 100 + 500 = 600, goodwill 8,300 - (600 + 930) = 6,770 and still balances. The sheet konsol
-- recorded does not agree with itself, so the accountant should complete the Business Combination; warn severity,
-- the build goes on.
--
...
VALUES
  ('BC-ZZG-ZZS-2026-03-15', 1, 'ZZ1199', 200, 0, 'Receivables (mistyped account)'),
  ...
  ('BC-ZZG-ZZS-2026-03-15', 5, 'ZZ3100', -500, 0, 'Retained earnings at acquisition (understated by 50)');
