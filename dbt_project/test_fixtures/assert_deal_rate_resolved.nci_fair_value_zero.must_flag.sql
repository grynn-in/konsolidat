-- Must-flag fixture for assert_deal_rate_resolved (konsol#204, row M4; PR #205 review): a submitted 80% deal that
-- measures NCI at 'full' and has NOT declared its nci_fair_value (0), whose header consideration_currency the group
-- has no governed Closing rate for, while every consideration line is in the group currency.
-- Gate: `+gold_business_combination_journal assert_deal_rate_resolved` must show `FAIL 1 assert_deal_rate_resolved`
-- (ZZX -> USD, FY2026 P3, kind business_combination). Predicted side effects: `FAIL` on
-- assert_acquisition_accounts_declared (the J12 share refusal of every deal below 100%, which stays: konsolidat#204,
-- and the J7 row naming the missing nci_fair_value) and, because that guard reads only the sources, the journal
-- SKIPs in this gate.
-- Why the guard must name the pair: the journal needs the consideration_currency rate for EVERY partial-share
-- 'full' deal (nci_fair_value_rated), whatever its declared figure. Before row M4 the guard asked for it only when
-- nci_fair_value > 0, so this deal was never checked for ZZX: the guard used a narrower condition than the journal.
-- The guard now asks whenever nci_measurement = 'full' and share_acquired_pct < 100, the journal's own condition.
--
-- assert_deal_rate_resolved.nci_fair_value.must_flag.sql's deal, verbatim except nci_fair_value 0 (not declared).
-- The consideration line stays 8,300 USD (the group currency translates at 1 and needs no rate). Only EUR -> USD
-- Closing/Average rates exist for FY2026 P3.
--
...
VALUES
  ('BC-ZZH-ZZT-2026-03-15', 'ZZH', 'ZZT', '2026-03-15', 80, 'ZZX', 8300, 650, 930, 0, 0, 0, 'ZZ-OP-ZZT-1', 'full', 0);
