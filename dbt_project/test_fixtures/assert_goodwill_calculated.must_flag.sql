-- assert_goodwill_calculated (PRD-11, konsolidat#198 row J2b): must FAIL with exactly 1 result.
-- `+gold_business_combination_journal assert_goodwill_calculated` BUILDs; the test flags the one deal.
--
-- Same group and chart as business_combination_100pct.sql: group ZZG (USD; IFRS, NCI partial, goodwill
-- Impairment only, costs Expense, measurement period Off, bargain purchase Recognise gain) with its
...
-- Entity ZZS (EUR) is acquired on 2026-03-15 (FY2026 P3) for 8,300 USD cash, BUT the deal carries no
...
--
-- The six deal tables do not exist live yet: the fixture creates the four the model reads (the costs table is
...
VALUES
  ('BC-ZZG-ZZS-2026-03-15', 'ZZG', 'ZZS', '2026-03-15', 100, 'USD', 8300, 0, 0, 0, 0, 0, 'ZZ-OP-ZZS-1', 'partial');
...
-- no acquired balances: the table is created above and stays empty
