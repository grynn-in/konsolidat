...
{#
    Consumes canonical stg_budget_entries for ERP-agnostic columns, for every
    ERP in `erp_sources` (it is already an empty typed relation when none is
    listed, macros/erp_sources.sql), so it is read unconditionally.

    Joins the D365 F&O adapter for budget_register_entry_recid and
    include_in_cash_flow (D365-specific fields) only when `d365_fo` is
    listed; otherwise both are 0.
#}
...
select
    ...
    {{ cast_to_int64('coalesce(d365.budget_register_entry_recid, 0)' if d365_joined else '0') }} as budget_register_entry_recid,
