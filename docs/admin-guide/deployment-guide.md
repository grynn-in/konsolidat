...
**Amount Basis.** Every trial balance upload declares what its amounts are, because the numbers alone cannot tell. Choose **Period movement** when the export holds only that period's activity (the usual "period" or "monthly" trial balance); **Year-to-date movement** when each row is the activity from the start of the fiscal year to the period end (a "YTD" export); **Period-end balance** when each row is the account's closing balance at the period end (a "balances" or "closing" trial balance, where balance-sheet accounts carry forward from year to year). The warehouse normalises every batch to period movements from the declared basis, so downstream reports are the same whichever an ERP exports. A wrong choice is a data error, not a display quirk: the next build names the batch (`assert_tb_submission_has_basis` when a batch has no basis, `assert_entity_has_one_amount_basis` when one entity's batches declare different bases) and stops until the batch is re-uploaded with the right one. For period-end balances the warehouse also books the year-end close itself — in each fiscal year's Closing period, the P&L balances are moved into the account the chart flags as retained earnings, so the next year opens with activity only — which needs a Closing period in the fiscal calendar and exactly one retained-earnings account in the chart (`assert_year_end_close_declared` names a year that has anything to close — any P&L account with a non-zero balance at its last period, even when revenue and expenses net to zero — and lacks either). The flagged retained-earnings account must be the one the ERP's files carry the result in: if the next year's first file shows the result in another equity code, retained earnings reads minus the result and that code plus it (`assert_year_end_close_carried` warns of the year).
