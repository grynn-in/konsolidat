...
| [Demo Data Source & Source-of-Truth Hygiene](PRD-DEMO-DATA-SOURCE.md) | Seeded `demo_data` connector as the default source (replaces hardcoded `d365_fo` fallback); empty-union guard; Dimension Mapping fixtures as crosswalk source of truth + regenerator guard. | Design — ready (all decisions resolved) |
