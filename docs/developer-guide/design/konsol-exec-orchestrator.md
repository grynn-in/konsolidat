...
```
Pipeline  (the DAG template; formerly Pipeline Definition)
   └─< Pipeline Step        type · depends_on · default params (formerly Step Definition)
...
|---|---|---|
| **Pipeline** | DAG template: ordered steps + dependencies + default params | new (formerly Pipeline Definition) |
| **Pipeline Run** | run aggregate + params | exists, thin → promoted |
| **Run Step** | per-step status/logs/deps/retry/params | **new — the missing core** (formerly Pipeline Step) |
