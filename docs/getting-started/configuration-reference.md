...
The add-in is served by Frappe; its server is the URL in the add-in's `manifest.xml` (`SourceLocation` and the other URLs; the shipped manifest points at `https://demo.konsolidat.com`, so replace them with your own server). For production, change those URLs, including the `AppDomains` entry (see the [Excel Task Pane Guide](../user-guide/excel-taskpane-guide.md#production-deployment)). Signing in on the task pane stores the session the worksheet functions use.
